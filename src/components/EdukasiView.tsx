          ...
          <article className="max-w-4xl mx-auto card-base bg-white overflow-hidden animate-scale-in p-0 border border-[#EFE6DA]">
            <div className="relative aspect-video bg-[#F5EDE3]">
              <button 
                ...
                className="absolute top-4 left-4 sm:top-6 sm:left-6 z-10 bg-white/90 backdrop-blur border border-[#EFE6DA] px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl text-[#3D1A0A] hover:text-[#7B1618] flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs font-bold uppercase cursor-pointer shadow-sm transition-colors"
              >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-8 md:p-12">
                <div className="flex flex-wrap items-center gap-2.5 sm:gap-4 text-[9px] sm:text-xs font-mono text-[#E0B060] font-bold uppercase tracking-widest mb-2 sm:mb-4">
            ...
            <div className="p-5 sm:p-8 md:p-12 text-[#5A4538] leading-relaxed space-y-5 text-sm sm:text-base md:text-lg whitespace-pre-wrap font-sans font-medium">
              {selected.content}
            </div>
          </article>
        ) : (
            ...
            {articles.length === 0 ? (
              ...
              articles.map(art => (
                ...
                >
                  ...
                  <div className="p-3.5 sm:p-6 flex-1 flex flex-col border-t border-[#EFE6DA]">
                    <div className="flex flex-wrap items-center gap-2 text-[8px] sm:text-[10px] font-mono text-[#C8973A] font-bold uppercase tracking-widest mb-2">
                      <span className="flex items-center gap-1"><User className="w-3 h-3" /> {art.author}</span>
                      <span className="w-1.5 h-1.5 rounded-full bg-[#EFE6DA] flex-shrink-0" />
                      <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {formatDate(art.createdAt)}</span>
                    </div>

                    <h3 className="font-serif text-sm sm:text-xl font-bold text-[#3D1A0A] leading-snug mb-2 group-hover:text-[#7B1618] transition-colors line-clamp-2">
                      {art.title}
                    </h3>

                    <p className="text-xs sm:text-sm text-[#7A6558] line-clamp-3 mb-4 flex-1 leading-relaxed">
                      {art.excerpt}
                    </p>

                    <div className="mt-auto pt-3 border-t border-[#EFE6DA]">
