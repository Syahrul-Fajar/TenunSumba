    ...
    <div id="home-view" className="animate-fade-in pb-20">
      ...
      <section className="relative flex items-center justify-center min-h-screen overflow-hidden pt-20">
        <div className="absolute inset-0 z-0 bg-black">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/50 to-[#FFFFFF]" />
        </div>
        ...
        <div className="card-base bg-white p-8 grid grid-cols-2 md:grid-cols-4 gap-8 text-center shadow-lg">
          {stats.map((s, i) => (
            <div key={i} className="py-2">
              ...
              <p className="text-[10px] font-mono uppercase tracking-widest text-[#64748B]">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-28 bg-[#FFFFFF]">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="font-serif font-bold text-3xl md:text-4xl text-[#1A1A1A] mt-3">Kualitas <span className="text-gradient">Autentik</span></h2>
            <p className="mt-4 max-w-lg mx-auto text-[#64748B]">Proses penciptaan dikurasi ketat dengan mempertahankan pakem murni tradisi.</p>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v, i) => (
              <div key={i} className="card-base p-8 text-center group bg-white">
                <div className="w-14 h-14 rounded-2xl mx-auto mb-6 flex items-center justify-center bg-[#7B1618]/5 border border-[#7B1618]/10 text-[#7B1618] group-hover:scale-110 group-hover:bg-[#7B1618] group-hover:text-white transition-all duration-300">
                  {v.icon}
                </div>
                <h3 className="font-serif font-bold text-lg text-[#1A1A1A] mb-3">{v.title}</h3>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5 mb-12">
            <div>
              ...
              <h2 className="font-serif font-bold text-3xl md:text-4xl text-[#1A1A1A] mt-3">Karya <span className="text-gradient">Unggulan</span></h2>
            </div>
          ...
          {featured.length === 0 ? (
            <div className="card-base p-20 text-center text-[#64748B] font-mono animate-pulse border-dashed">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {featured.map(p => (
                ...
                >
                  <div className="relative overflow-hidden aspect-[4/5] bg-[#F8FAFC]">
                    <img 
                      src={p.image} 
                      alt={p.title} 
                    ...
                    <div className="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                      <span className="bg-white/90 backdrop-blur text-[#1A1A1A] px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest flex items-center justify-between shadow-lg">
                  ...
                  <div className="p-3.5 sm:p-6 flex-1 flex flex-col bg-white">
                    <span className="text-[8px] sm:text-[10px] font-mono uppercase tracking-widest text-[#7B1618] mb-1.5">{p.category}</span>
                    ...
                    <div className="pt-3 border-t border-[#F1F5F9] flex justify-between items-center">
                      ...
                      <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-[#FFFFFF] flex items-center justify-center group-hover:bg-[#7B1618] group-hover:text-white transition-colors text-[#7B1618]">
