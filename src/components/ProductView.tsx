      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            ...
            <h1 className="font-serif font-extrabold text-4xl text-[#3D1A0A] mt-2">Katalog <span className="text-gradient">Tenun</span></h1>
            <p className="mt-3 text-sm text-[#7A6558] max-w-lg">Eksplorasi mahakarya tenun ikat yang diciptakan dengan dedikasi tinggi oleh para maestri Sumba.</p>
          </div>
          ...
          <div className="lg:w-1/4 flex-shrink-0">
            <div className={`card-base p-6 sticky top-28 ${mobileFilterOpen ? 'block' : 'hidden lg:block'}`}>
              <div className="flex items-center justify-between mb-6 lg:hidden">
                <h3 className="font-bold text-[#3D1A0A] flex items-center gap-2"><Filter className="w-4 h-4 text-[#C8973A]"/> Filter</h3>
                <button onClick={() => setMobileFilterOpen(false)} className="text-[#7A6558] hover:text-[#3D1A0A]"><X className="w-5 h-5"/></button>
              </div>

              {/* Pencarian */}
              <div className="mb-8">
                <label className="block text-[10px] font-mono font-bold uppercase tracking-widest text-[#7A6558] mb-3">Pencarian Motif</label>
                <div className="relative">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8973A]" />
                  <input 
                    type="text" 
              ...
              {/* Kategori */}
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase tracking-widest text-[#7A6558] mb-3">Klasifikasi Kategori</label>
                    ...
                    <button 
                      ...
                      className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        activeCategory === cat 
                          ...
                          : 'text-[#7A6558] hover:bg-[#FBF8F4] hover:text-[#3D1A0A] border border-transparent'
                      }`}
                    >
                      {cat}
            ...
            <button 
              ...
              className="lg:hidden w-full card-base px-4 py-3 flex items-center justify-center gap-2 text-sm font-bold text-[#3D1A0A]"
            >
              <SlidersHorizontal className="w-4 h-4 text-[#C8973A]" /> Buka Panel Filter
            </button>
          </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-3 border-t border-[#EFE6DA]">
                        ...
                        {isAdmin && (
                          <div className="flex items-center gap-1.5 z-10" onClick={e => e.stopPropagation()}>
                            <button onClick={(e) => { e.stopPropagation(); setEditingProduct({...p}); setIsFormOpen(true); }} className="p-2 bg-[#FBF8F4] hover:bg-[#EFE6DA] rounded-lg text-[#7A6558] hover:text-[#3D1A0A] transition-all border border-[#EFE6DA]">
      ...
      {isFormOpen && editingProduct && isAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget) setIsFormOpen(false); }}>
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col animate-scale-in">
            <div className="flex items-center justify-between p-5 bg-[#FBF8F4] border-b border-[#EFE6DA] flex-shrink-0">
              <h3 className="font-serif text-lg font-bold text-[#3D1A0A]">{editingProduct.id ? 'Modifikasi Karya Tenun' : 'Registrasi Produk Baru'}</h3>
              <button onClick={() => setIsFormOpen(false)} className="text-[#7A6558] hover:text-red-600 transition-colors"><X className="w-5 h-5"/></button>
            </div>
            ...
            <form onSubmit={handleSaveProduct} className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Nama Kain *</label>
                  <input type="text" required value={editingProduct.title||''} onChange={e=>setEditingProduct({...editingProduct,title:e.target.value})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors" />
                </div>
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Kode SKU *</label>
                  <input type="text" required value={editingProduct.code||''} onChange={e=>setEditingProduct({...editingProduct,code:e.target.value})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors" />
                </div>
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Klasifikasi</label>
                  <select value={editingProduct.category||'Kain Tenun'} onChange={e=>setEditingProduct({...editingProduct,category:e.target.value})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors">
                    {CATEGORIES.slice(1).map(c=><option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Harga Dasar (IDR) *</label>
                  <input type="number" required value={editingProduct.price||''} onChange={e=>setEditingProduct({...editingProduct,price:Number(e.target.value)})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Akses URL Visual *</label>
                  <input type="url" required value={editingProduct.image||''} onChange={e=>setEditingProduct({...editingProduct,image:e.target.value})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-[#7A6558] mb-1.5">Filosofi Historis Produk</label>
                  <textarea rows={4} value={editingProduct.description||''} onChange={e=>setEditingProduct({...editingProduct,description:e.target.value})} className="w-full px-4 py-3 text-sm bg-[#FBF8F4] border border-[#EFE6DA] rounded-xl text-[#3D1A0A] focus:outline-none focus:border-[#C8973A] transition-colors resize-none" />
                </div>
              </div>

              <div className="flex justify-end gap-3 pt-5 border-t border-[#EFE6DA]">
